  ...
  return (
    <div className="py-16 flex flex-col items-center justify-center gap-4">
      ...
      <div className="text-center">
        <h3 className="text-slate-300 font-semibold">No active allocations to return</h3>
        <p className="text-xs text-slate-500 mt-1">
          All assets are currently in stock or already returned. Allocate an asset first.
        </p>
      </div>
    </div>
  );
...
export default function Returns() {
  ...
  const fetchAllocations = useCallback(async () => {
    ...
    try {
      const res = await allocationAPI.getActive({ limit: 200 });
  ...
  return (
    <div className="flex flex-col min-h-full">
      <Navbar title="Return Management" subtitle="Process asset returns from employees" />
      <div className="flex-1 p-6 space-y-4 animate-fade-in">

        <div className="card p-4 bg-sky-500/5 border border-sky-500/20 flex items-start gap-3">
          <RotateCcw className="w-4 h-4 text-sky-400 mt-0.5 flex-shrink-0" />
          <p className="text-sm text-sky-400">
            Select an actively allocated asset below to process its return.
          </p>
        </div>
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              ...
              <tbody className="divide-y divide-slate-800">
                ...
                {!loading && error && <tr><td colSpan={6}><ErrorMessage message={error} onRetry={fetchAllocations} /></td></tr>}
                ...
                {!loading && allocations.map((a) => (
                  <tr key={a.id} className="table-row-hover">
                    ...
                    <td className="px-4 py-3">
                      <button
                        onClick={() => { setSelectedAsset(a); setNotes(''); }}
        ...
        {selectedAsset && (
          <form onSubmit={handleReturn} className="space-y-4">
            <div className="p-3 rounded-lg bg-slate-800 border border-slate-700 space-y-1">
              <p className="text-xs text-slate-500">Returning asset</p>
              <p className="font-semibold text-slate-200">{selectedAsset.asset_name}</p>
              <p className="text-xs text-slate-500 font-mono">{selectedAsset.serial_number} · {selectedAsset.category_name}</p>
              <p className="text-xs text-slate-400 mt-1">
                From: <span className="text-slate-200">{selectedAsset.employee_name}</span>
                <span className="text-slate-600 mx-1">·</span>
                <span className="text-slate-500">{selectedAsset.department}</span>
              </p>
            </div>
            <div>
              <label className="label">Return Notes <span className="text-slate-500 font-normal">(optional)</span></label>
              <textarea className="input resize-none" rows={3}
                placeholder="Condition on return, reason, remarks..."
